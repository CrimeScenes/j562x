...
if Shared.Library.Memory.Settings.IsActive == true then
    local Memory
    ...
    task.spawn(function()
        while task.wait(1) do
            local minMemory = Shared.Library.Memory.Configuration.Start
            local maxMemory = Shared.Library.Memory.Configuration.End
...
local function isWithinViewEra(position)
    ...
    local fovHeight = Shared.Library.ViewEra.Vertical * 100
    local fovWidth = Shared.Library.ViewEra.Horizontal * 100
            ...
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    if onScreen and isWithinViewEra(part.Position) then
                        ...
                        if os.clock() - lastClickTime >= Shared.Library.TriggerBot.Settings.ResetTime then  
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Shared.Library.TriggerBot.Settings.Keybind then
        isToggled = true
    end
end)
...
local function UpdateFOV()
    ...
    local reach = Shared.Library.PointAssist.Reach
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = Shared.Library.PointAssist.Reach_Visibility
...
Mouse.KeyDown:Connect(function(Key)
    ...
    if key == Shared.Library.ActivationKey:lower() then
        if Shared.Library.PointAssist.IsActive then
            if IsTargeting then
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if Shared.Library.PointAssist.Resolver then
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                  
                    predictedPosition = BodyPart.Position + (moveDirection * Vector3.new(
                        Shared.Library.PointAssist.VelocityCompensation.X,
                        Shared.Library.PointAssist.VelocityCompensation.Y,
                        Shared.Library.PointAssist.VelocityCompensation.Z
                    ))
                end
            else
            ...
            if predictedPosition then
                ...
                Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, Shared.Library.PointAssist.Stabilization)
            end
        
        

            if Shared.Library['Silent Precision'].IsActive and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                ...
                Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * Vector3.new(
                    Shared.Library['Silent Precision'].TimeDistortion.X, 
                    Shared.Library['Silent Precision'].TimeDistortion.Y, 
                    Shared.Library['Silent Precision'].TimeDistortion.Z
                ))
            end
...
local function updateFov()
    local settings = Shared.Library['Silent Precision'].VoidLimits
    clearFovParts()
...
local function Death(Plr)
    if Plr.Character and Plr.Character:FindFirstChild("BodyEffects") then
        ...
        local ko = bodyEffects:FindFirstChild(Shared.Library.BodyEffects)
...
local function isPartInFovAndVisible(part)
    if not Shared.Library.PointAssist.IsActive or not IsTargeting or not TargetPlayer then
        return false
    end

    -- Dynamically calculate the FOV based on X, Y, Z values
    local dynamicFovSize = calculateFov(Shared.Library['Silent Precision'].VoidLimits.X, 
                                         Shared.Library['Silent Precision'].VoidLimits.Y, 
                                         Shared.Library['Silent Precision'].VoidLimits.Z)
...
local function isPartVisible(part)
    if not Shared.Library['Silent Precision'].WallCheck then 
        return true
    end
...
local OldTimeDistortion = Shared.Library['Silent Precision'].TimeDistortion  
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        local distortionX = Shared.Library['Silent Precision'].TimeDistortion.X
        local distortionY = Shared.Library['Silent Precision'].TimeDistortion.Y
        local distortionZ = Shared.Library['Silent Precision'].TimeDistortion.Z
        ...
        if adjustedVelocity.Y < -30 and Shared.Library['Silent Precision'].Resolver then
            Shared.Library['Silent Precision'].TimeDistortion = { X = 0, Y = 0, Z = 0 } 
            return adjustedVelocity
        elseif adjustedVelocity.Magnitude > 50 and Shared.Library['Silent Precision'].Resolver then
            return player.Character:FindFirstChild("Humanoid").MoveDirection * 16 * distortionX  
        else
            Shared.Library['Silent Precision'].TimeDistortion = OldTimeDistortion 
            return adjustedVelocity
        end
    end
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    if input.KeyCode == Enum.KeyCode[Shared.Library.ActivationKey:upper()] and Shared.Library.pointassist.Method == "hold" then
...
RunService.RenderStepped:Connect(function()
    ...
    if Shared.Library['Silent Precision'].IsActive and IsTargeting then  
        UpdateFOV()  
        ...
        if TargetPlayer then
            if TargetPlayer.Character then
                ...
                if closestPart and closestPoint then
                    ...
                    local timeDistortionX = Shared.Library['Silent Precision'].TimeDistortion.X
                    local timeDistortionY = Shared.Library['Silent Precision'].TimeDistortion.Y
                    local timeDistortionZ = Shared.Library['Silent Precision'].TimeDistortion.Z
    ...
    task.spawn(function()
        while task.wait(0.1) do
            if Shared.Library['Silent Precision'].IsActive then
                Fov.Visible = IsTargeting and Shared.Library['Silent Precision'].VoidLimits.FovVisible 
            end
        end
    end)
...
if Shared.Library.TargetControl.BlockGroundHits == true then
    local function CheckNoGroundShots(Plr)
        if Shared.Library.TargetControl.BlockGroundHits and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            pcall(function()
                local TargetVelv5 = Plr.Character:FindFirstChild(Shared.Library.Silent and Shared.Library.Silent)
                if TargetVelv5 then
